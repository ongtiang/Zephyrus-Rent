...
<head>
    ...
    <title>Zephyrus Rental - My Account</title>
    ...
    <?php
        if($_SERVER['REQUEST_METHOD'] == 'POST'){
            ...
            if (isset($_POST['editsubmit'])) {
                // Assuming you've sanitized and validated the input
                $fname = $_POST['fname'];
                $lname = $_POST['lname'];
                $age = $_POST['age'];
            
                // Check if age is greater than 1
                if ($age > 1) {
                    $dob = $_POST['dob'];
                    $mail = $_POST['mail'];
                    $phone = $_POST['phone'];
                    ...
                    $query = "UPDATE account SET fname = ?, lname = ?, age = ?, dob = ?, email = ?, phone = ?, password = ?, city = ?, state = ? WHERE id = ?";
            
                    $stmt = $con->prepare($query);
                    $stmt->bind_param("sssssssssi", $fname, $lname, $age, $dob, $mail, $phone, $password, $city, $state, $_SESSION['id']);
            
                    if ($stmt->execute()) {
                        echo "<script>alert('Data Change Successfully');</script>";
                    } else {
                        echo "<script>alert('Data Change Failed: " . $stmt->error . "');</script>";
                    }
            
                    $stmt->close();
                } else {
                    echo "<script>alert('Please enter a valid age greater than 1.');</script>";
