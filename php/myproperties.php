...
<head>
    ...
    <title>Zephyrus Rental : MyProperties</title>
...
<body>
    ...
    <?php 
        ...
        if (isset($_POST['accept'])) {
            ...
            // Query the inbox to get the price and type
            $inboxQuery = "SELECT price, type FROM inbox WHERE msgid = $msgid";
            $inboxResult = mysqli_query($con, $inboxQuery);
            $rowInbox = mysqli_fetch_array($inboxResult);
        
            if ($rowInbox) {
                $price = $rowInbox['price'];
                $type = $rowInbox['type'];
        
                // Insert the current date, price, and type into the 'sales' table
                $insertSalesQuery = "INSERT INTO sales(price, type, date) VALUES($price, '$type', NOW())";
                mysqli_query($con, $insertSalesQuery);
        
                // Continue with the rest of your code to remove the request from the inbox
                $deleteInboxQuery = "DELETE FROM inbox WHERE msgid = $msgid";
                mysqli_query($con, $deleteInboxQuery);
                ...
                $deletePropertyQuery = "DELETE FROM properties WHERE id = $P_id";
                mysqli_query($con, $deletePropertyQuery);
        
            } else {
                // Handle the case where the price couldn't be retrieved
                ...
                while($row = mysqli_fetch_array($result)){
                    ...
                    <tr class="box">
                        <td class="box-content" ><?php echo $n ; ?></td>
                            ...
                            <td class="box-content" id = 'medis'>RM<?php echo $row['price'] ; ?></td>
                ...
                while($row = mysqli_fetch_array($result)){ 
                    ...
                    ?> 
                        ...
                        <td class ='box-items'><form action ="" onsubmit = 'return transaction()' method = "post">
                            ...
                            <input type = 'submit' value = 'Payment Done' name = 'accept' id ='accept'>
                        </form></td>   
